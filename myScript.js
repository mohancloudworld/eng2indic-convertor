...
function init()
{
    ...
    devanagari_dict_mod = extend(devanagari_dict);
    telugu_dict_mod     = extend(telugu_dict);
    kannada_dict_mod    = extend(kannada_dict);
    gujarati_dict_mod   = extend(gujarati_dict);
    tamil_dict_mod      = extend(tamil_dict);
    bengali_dict_mod    = extend(bengali_dict);
    english_dict_mod    = extend(english_dict);

    // reverse dictionaries
    devanagari_dict_rev = reverse(devanagari_dict);
    telugu_dict_rev     = reverse(telugu_dict);
    kannada_dict_rev    = reverse(kannada_dict);
    gujarati_dict_rev   = reverse(gujarati_dict);
    tamil_dict_rev      = reverse(tamil_dict);
    bengali_dict_rev    = reverse(bengali_dict);
    english_dict_rev    = reverse(english_dict);
}
...
function convert2IndicScript(inp_txt, is_IAST, indicScript, modeStrict, reverse, preferASCIIDigits)
{
    ...
    if(reverse){
        ...
        if(indicScript == "Devanagari"){lang_dict = devanagari_dict;}
        else if(indicScript == "Telugu"){lang_dict = telugu_dict;}
        else if(indicScript == "Kannada"){lang_dict = kannada_dict;}
        else if(indicScript == "Gujarati"){lang_dict = gujarati_dict;}
        else if(indicScript == "Tamil"){lang_dict = tamil_dict;}
        else if(indicScript == "Bengali"){lang_dict = bengali_dict;}
        else {lang_dict = english_dict;}
    } 
...
function convert2ITRANS(inp_txt)
{
    ...
    while (indx <  inp_txt.length){
        [blk, blkLen, Type, insideTag_p] = convertNextBlk2ITRANS(iast2itrans_dict, inp_txt.substring(indx), insideTag_p);
        out += blk;
        if(Type == "NoMatch"){
            indx += 1;
        }
